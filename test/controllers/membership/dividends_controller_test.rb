...
module Membership
  class DividendsControllerTest < ActionDispatch::IntegrationTest
    ...
    test 'should redirect to sign in when not authenticated for dividend' do
      sign_out @user
      get membership_dividend_path @dividend

      assert_redirected_to new_user_session_path
    end

    test 'should redirect back to dividends if not authorised for dividend' do
      assert_not_equal @not_my_dividend.member, @member

      get membership_dividend_path @not_my_dividend

      assert_redirected_to membership_dividends_path
      assert_equal 'That\'s not your dividend. 🤨', flash[:alert]
      assert_predicate @not_my_dividend.reload, :issued?
    end

    test 'should get dividend' do
      get membership_dividend_path @dividend
    ...
    test 'should redirect to sign in when not authenticated for pay out' do
      sign_out @user
      patch pay_out_membership_dividend_path(@dividend)
    ...
    test 'should redirect back to dividends if not authorised for pay out' do
      ...
      patch pay_out_membership_dividend_path(@not_my_dividend)
    ...
    test 'should redirect back to dividends if not in issued state for pay out' do
      assert_predicate @payed_out_dividend, :pending_pay_out?

      patch pay_out_membership_dividend_path(@payed_out_dividend)

      assert_redirected_to membership_dividends_path
      assert_equal 'Dividend already updated.', flash[:alert]
      assert_predicate @payed_out_dividend.reload, :pending_pay_out?
    end
    ...
    test 'should update dividend to pay out' do
      ...
      assert_predicate @dividend.reload, :pending_pay_out?
    end

    test 'should redirect to sign in when not authenticated for recontributed' do
      sign_out @user
      patch recontribute_membership_dividend_path(@dividend)
    ...
    test 'should redirect back to dividends if not authorised for recontributed' do
      ...
      patch recontribute_membership_dividend_path(@not_my_dividend)
    ...
    test 'should redirect back to dividends if not in issued state for recontributed' do
      ...
      patch recontribute_membership_dividend_path(@payed_out_dividend)
    ...
    test 'should update dividend to recontributed' do
      ...
      assert_predicate @dividend.reload, :manually_recontributed?
    end
  end
end
