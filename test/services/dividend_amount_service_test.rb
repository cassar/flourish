...
class DividendAmountServiceTest < ActiveSupport::TestCase
  test 'amount_in_aud_base_units' do
    assert_equal 5_000, DividendAmountService.new(
      total_pool_in_aud_base_units: 10_000,
      member_count: 2
    ).amount_in_aud_base_units
  end

  test 'returns 0 if total_pool_in_aud_base_units is 0' do
    assert_equal 0, DividendAmountService.new(
      total_pool_in_aud_base_units: 0,
      member_count: 2
    ).amount_in_aud_base_units
  end
